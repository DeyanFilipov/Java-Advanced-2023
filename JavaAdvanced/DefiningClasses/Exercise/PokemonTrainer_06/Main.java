package JavaAdvanced.DefiningClasses.Exercise.PokemonTrainer_06;
...
public class Main {
    public static void main(String[] args) {
        ...
        while (!command.equals("End")) {
            for (Map.Entry<String, Trainer> trainer : trainerMap.entrySet()) {
                if (!trainer.getValue().containsElement(command)) {
                    trainer.getValue().setLeftMinus10();
                } else {
        ...
        trainerMap.values().stream().sorted((n1, n2) -> Integer.compare(n2.getBadges(), n1.getBadges())).forEach(item -> System.out.printf("%s %d %d\n", item.getName(), item.getBadges(), item.getAvailablePokemons().size()));
    }
}
